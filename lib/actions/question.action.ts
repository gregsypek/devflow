...
export async function getQuestions(
  ...
  if (query) {
    filterQuery.$or = [
      { title: { $regex: new RegExp(query, "i") } }, // don't care about capitalization
  ...
  switch (filter) {
    case "newest":
      sortCriteria = { createdAt: -1 }; // from newest to oldest
      break;
    case "unanswered":
  ...
  try {
    ...
    return {
      success: true,
      // JSON.parse... is for compatibility with next server actions
